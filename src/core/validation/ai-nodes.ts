 ...
 * Ported from n8n-mcp/src/services/ai-node-validator.ts
 *
 * Key Features:
...
import { isAIToolSubNode, validateAIToolSubNode, type AIValidationIssue } from './ai-tool-validators.js';

// Re-export for convenience
export type { AIValidationIssue } from './ai-tool-validators.js';
...
/**
 * AI Connection Types
 * These are the special connection types used by AI/LangChain nodes
 */
...
function checkIfStreamingTarget(
  ...
  for (const conn of mainConnections) {
    ...
    if (normalizedType === 'nodes-langchain.chatTrigger') {
      const options = sourceNode.parameters?.options as Record<string, unknown> | undefined;
...
export function validateAIAgent(
  ...
  const nodeName = node.name || 'Unnamed AI Agent';
  const incoming = reverseConnections.get(nodeName) || [];
  ...
  const hasOwnStreamingEnabled = options?.streamResponse === true;

  if (isStreamingTarget || hasOwnStreamingEnabled) {
    // Check if AI Agent has any main output connections
    const agentMainOutput = (workflow.connections[nodeName] as Record<string, unknown> | undefined)?.main;
    if (agentMainOutput && Array.isArray(agentMainOutput) && agentMainOutput.flat().some((c: unknown) => c)) {
  ...
  if (params.maxIterations !== undefined) {
    ...
    } else if (params.maxIterations < 1) {
      issues.push({
        severity: 'error',
        nodeId: node.id,
        nodeName,
...
export function validateChatTrigger(
  ...
  const outgoingMain = nodeConnections?.main;
  ...
  const firstConnection = outgoingMain[0][0] as { node?: string } | undefined;
  if (!firstConnection || !firstConnection.node) {
    return issues;
  }
  ...
  const targetType = NodeTypeNormalizer.normalizeToShortForm(targetNode.type);
  ...
  if (responseMode === 'streaming') {
    ...
    if (targetType !== 'nodes-langchain.agent') {
      ...
      const agentConnections = workflow.connections[targetNodeName] as Record<string, unknown[][]> | undefined;
      const agentMainOutput = agentConnections?.main;
      if (agentMainOutput && Array.isArray(agentMainOutput) && agentMainOutput.flat().some((c: unknown) => c)) {
        issues.push({
          severity: 'error',
...
export function validateBasicLLMChain(
  ...
  const issues: AIValidationIssue[] = [];
  const nodeName = node.name || 'Unnamed Basic LLM Chain';
  const incoming = reverseConnections.get(nodeName) || [];
  const params = node.parameters || {};
 ...
 *
 * This is the main entry point called by the validator
 */
export function validateAISpecificNodes(
  ...
  for (const node of workflow.nodes) {
    ...
    // Validate AI tool sub-nodes (12 types)
