 ...
 * 
 * Validates and fixes fixedCollection structures in n8n nodes.
 * Prevents the "propertyValues[itemName] is not iterable" error.
 * Ported from n8n-mcp/src/utils/fixed-collection-validator.ts
 */

import type {
  FixedCollectionPattern,
...
export class FixedCollectionValidator {
   ...
   */
  private static hasInvalidStructure(config: NodeConfig, pattern: string): boolean {
    ...
    for (const part of parts) {
      ...
      if (typeof nextValue !== 'object' || nextValue === null) {
        // If we have more parts to traverse but current value is not an object
          ...
          if (this.isNodeConfig(conditions) && 'values' in conditions) {
            ...
            fixedConfig.rules = {
              values: Array.isArray(values)
                ...
                : [{ conditions: values, outputKey: 'output1' }],
            };
          } else if (conditions) {
            fixedConfig.rules = {
