...
workflowsCmd
  ...
  .addHelpText('after', `
Confidence Levels:
  high     Only safe, certain fixes (default behavior is cautious)
  medium   Include likely-correct fixes (default)
  low      Include experimental fixes (review carefully!)

Behavior:
  Default is --dry-run (preview only, no changes made)
  Use --apply to actually modify the workflow
  Use --save to save fixed version locally instead of updating server

⚠️  Always review fixes before applying! Use --dry-run first.

Examples:
  n8n workflows autofix abc123                       # Preview fixes
  n8n workflows autofix abc123 --apply --force       # Apply without prompts
  n8n workflows autofix abc123 --save fixed.json     # Save locally
  n8n workflows autofix abc123 --confidence low      # Include experimental
`)
  .action(async (id, opts) => {
...
templatesCmd
  ...
  .option('--json', 'Output as JSON')
  .addHelpText('after', `
🌐 Source: Templates are fetched from n8n.io template library.
   Requires internet connection.

Search Tips:
  • Use service names: "slack", "notion", "openai"
  • Use workflow types: "backup", "sync", "notification"
  • Browse categories: "automation", "ai", "data"

Examples:
  n8n templates search "openai"
  n8n templates search "slack notification" --limit 20
  n8n templates search "data sync" --save templates.json

💡 Get full template: n8n templates get TEMPLATE_ID --save workflow.json
`)
  .action(async (query, opts) => {
